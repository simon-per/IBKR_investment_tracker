...
function SourceBadge({ row }: { row: DividendSecurityRow }) {
  if (row.payouts === 0 && row.forecast_payouts > 0) {
    return (
      <span
        className="rounded-full bg-violet-100 px-2.5 py-1 text-xs font-medium text-violet-800 dark:bg-violet-950/60 dark:text-violet-200"
  ...
  if (row.source === 'ibkr') {
    return (
      <span
        className="rounded-full bg-teal-100 px-2.5 py-1 text-xs font-medium text-teal-800 dark:bg-teal-950/60 dark:text-teal-200"
  ...
  return (
    <span
      className="rounded-full bg-amber-100 px-2.5 py-1 text-xs font-medium text-amber-800 dark:bg-amber-950/60 dark:text-amber-200"
      title={
        row.source === 'mixed'
    ...
    >
      {row.source === 'mixed' ? 'Mixed' : 'Estimated'}
    </span>
  )
}
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        {isLoading ? (
          ...
          <div className="flex h-40 items-center justify-center text-sm text-muted-foreground">
            No dividends recorded {year === 'all' ? 'yet' : `for ${year}`}. They arrive with the
            IBKR sync; estimates for earlier years come from the dividend sync.
          </div>
        ) : (
          <>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border text-xs text-muted-foreground">
                    ...
                    <th className="px-2 py-1.5 text-right font-medium">Yield (TTM)</th>
                  </tr>
                </thead>
                <tbody>
                  {securities.map((row) => (
